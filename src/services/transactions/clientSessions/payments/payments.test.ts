...
getTransactionProviderList().forEach(provider => {
  describe(`TransactionClientSession.Payments ${provider}`, () => {
    ...
    describe('Syntax Validation', () => {
      it('should validate start syntax and types', async () => {
        ...
        await expectSyntaxValidation(() =>
          client.transactions.clientSessions.payments.start({
      ...
      it('should validate verify syntax and types', async () => {
        await expectSyntaxValidation(() =>
          client.transactions.clientSessions.payments.verify({
    ...
    describe('Integration', () => {
      it('should be able to start a payment session', async () => {
        ...
        const session = await client.transactions.clientSessions.payments.start(
          {
            transactionProvider: provider,
            data: minimalData,
          },
      ...
      it('should be able to verify a payment session', async () => {
        const minimalData = {
          amount: BigInt(faker.number.int({ min: 100, max: 10000 })),
          currency: CURRENCY.USD,
          merchantInternalCustomerCode: `test-cust-${faker.string.alphanumeric(10)}`,
